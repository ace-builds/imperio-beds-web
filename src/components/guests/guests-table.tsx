...
import { useGuest, useGuestsWithStats, type GuestWithStats } from '@/hooks/use-guests'
...
function GuestSubtitle({ hotelId, guestId }: { hotelId: string; guestId: string }) {
  const { data } = useGuest(hotelId, guestId)
  const latestNote = data?.notes[0]
  if (!latestNote) return null
  return <p className="max-w-56 truncate text-xs text-muted-foreground">{latestNote.body}</p>
}

export function GuestsTable({ hotelId, canManage }: { hotelId: string; canManage: boolean }) {
  ...
  const { data: guests, isLoading } = useGuestsWithStats(hotelId, search || undefined)
  ...
  const pageRows = (guests ?? []).slice((currentPage - 1) * PAGE_SIZE, currentPage * PAGE_SIZE)
  ...
  return (
    <div className="flex flex-col gap-4">
          ...
          <TableBody>
            ...
            {pageRows.map((guest: GuestWithStats) => (
              <TableRow key={guest.id}>
                ...
                <TableCell>
                  {guest.lastVisit
                    ? guest.lastVisit.toLocaleDateString('en-US', {
                        month: 'short',
                        day: 'numeric',
                        year: 'numeric',
                      })
      ...
      <GuestFormDialog open={formOpen} onOpenChange={setFormOpen} hotelId={hotelId} guest={editing} />
