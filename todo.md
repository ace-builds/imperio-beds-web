...
- [x] Auth survives an offline cold start (2026-08-04) — `beforeLoad` used to redirect to `/login` whenever `getSession()` didn't return a session, which included "the API is unreachable" (it *throws* there, it doesn't return an `error`), so rebooting a front-desk machine with no network locked the user out of an app whose data is local. `lib/session.ts` now only treats a 200-with-no-session as signed out; anything else falls back to a cached session (kept in localStorage, honouring the server's own `expiresAt`) and marks the route context `isOffline`, revalidating on `online` or socket reconnect. `stores/current-hotel.ts` is persisted for the same reason — the active hotel otherwise comes from `/me/hotel-access`, which is unavailable offline
- [x] Desktop (Tauri) build authenticates by bearer token (2026-08-04) — the packaged app's origin is `tauri://localhost`, which makes BetterAuth's session cookie third-party (WKWebView blocks those), so cookie auth would have worked in `desktop:dev` and silently failed in `desktop:build`. `lib/auth-token.ts` stores the token from the `set-auth-token` header when `isTauri()`, and `auth-client.ts`/`api/client.ts`/`socket.ts` send it as `Authorization: Bearer`; the browser build is untouched and keeps using the httpOnly cookie
