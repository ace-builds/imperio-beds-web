...
`src/routeTree.gen.ts` is auto-generated by the TanStack Router Vite plugin and gitignored. If it's missing (e.g. right after a fresh clone), running `dev` or `build` regenerates it — plain `tsc -b` alone won't, since it only runs the type-checker, not the Vite plugin.
...
Vitest is the intended test runner (shares Vite's config/transform pipeline) but isn't installed yet — add it (`pnpm add -D vitest @testing-library/react jsdom`) when there's real component/hook logic worth testing, rather than scaffolding empty test files now.
